...
\begin{theorem} \label{thm:shkredov}
...
We're now able to prove Theorem \ref{thm:shkredov}.

\begin{proof}[Proof of Theorem \ref{thm:shkredov}]
Define the difference and sum sets with \(D = \left\lvert A-A \right\rvert \) and \(S = \left\lvert A+A \right\rvert \).
Observe that
\[
...
\textbf{BELOW}
\[
    \left\lvert A + A \right\rvert \geq  \frac{ c \left\lvert A \right\rvert ^{2}}{\log \left( \left\lvert A \right\rvert  \right) } 
.\]
...
We require a lemma and a prerequisite theorem.
