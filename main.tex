...
and therefore
\[
    \sum _{\substack{ p \leq N \\ p \text{ prime} \\ i \leq \log _{2} \left( N \right)  }} \frac{\log \left( p \right) }{p} \leq  \frac{\log \left( N! \right) }{N} + \frac{\psi(N)}{N} 
.\]

Finally, via a Riemann sum,
\[
    \log \left( N! \right) = \sum_{i=1}^{N} \log \left( i \right) = \int_{2}^{N}  ~\mathrm{d} x 
\]
\[
